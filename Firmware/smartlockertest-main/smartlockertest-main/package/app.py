     ...
     if request.method == 'POST':
        ...
        if user:
            ...
            if available_lockers:
                ...
                # Tạo một tài liệu Firestore cho bảng lockers
                db_firestore = firestore.client()
                locker_doc_ref = db_firestore.collection("lockers").document(selected_locker_id)
                locker_data = {
                            'locker_id': selected_locker_id,
                            'status': 'on'
                        }
                locker_doc_ref.set(locker_data)
    ...
    try:
        if request.method == 'POST':
            ...
            if result:
                ...
                db.commit()
                db_firestore = firestore.client()
                locker_doc_ref = db_firestore.collection("lockers").document(locker_id)
                locker_data = {
                    'locker_id': locker_id,
                    'status': 'off'
                }
